...
public class QueryTransformer {
     ...
     *
     * <p>
     * If configured (>0), the caller-side hint-timeout (Future#get) is clamped to this
    ...
    /**
     * Local soft-cooldown window for transient QTX LLM failures (model loading / timeouts).
     *
     * <p>
     * We intentionally avoid tripping the global breaker for these transitional failures
     * because QueryTransformer is an optional stage. Instead we step down (fail-soft)
     * for a short window to avoid thrash.
     * </p>
     */
    private final java.util.concurrent.atomic.AtomicLong qtxSoftCooldownUntilMs = new java.util.concurrent.atomic.AtomicLong(0L);
    ...
    private void startQtxSoftCooldown(String reason, NightmareBreaker.FailureKind kind, Throwable err, long cooldownMs) {
        long ms = Math.max(0L, cooldownMs);
        if (ms <= 0L) {
            return;
        }
        long now = System.currentTimeMillis();
    ...
    private long qtxSoftCooldownMsFor(NightmareBreaker.FailureKind kind, boolean modelLoading) {
        long base = Math.max(0L, llmTimeoutOpenHintMs);
        if (base <= 0L) {
            base = 3000L;
        }
        ...
        if (modelLoading) {
            // Model warm-up/loading is typically short; avoid hammering while it stabilizes.
            return Math.max(base, 5000L);
        }
    ...
    private BypassDecision computeBypassDecision() {
        ...
        // QTX local soft cooldown (transient failures): step-down routing without breaker trip.
    ...
    private String runLLM(String prompt) {
        ...
        try {
            ...
            if (text == null || text.isBlank()) {
                ...
                startQtxSoftCooldown("blank", NightmareBreaker.FailureKind.UNKNOWN, null, llmTimeoutOpenHintMs);
                try {
                ...
                try {
                    ...
                    dd.put("promptLen", (prompt != null ? prompt.length() : 0));
                    dd.put("promptSha1", com.abandonware.ai.agent.integrations.TextUtils.sha1(prompt));
                    dd.put("cooldownMs", llmTimeoutOpenHintMs);
                    emitQtx(
                            DebugEventLevel.WARN,
                            "qtx.blank",
            ...
            if (FriendShieldPatternDetector.looksLikeSilentFailure(text)) {
                ...
                startQtxSoftCooldown("friendshield", NightmareBreaker.FailureKind.UNKNOWN, null, llmTimeoutOpenHintMs);
                try {
            ...
            // Soft-failure ladder: for transient errors (model loading / timeout / 5xx / 429),
            // do NOT trip the breaker in this optional stage. Instead, start a short cooldown
            ...
            if (isSoftTransientForQtx(kind, modelLoading)) {
                ...
                startQtxSoftCooldown(modelLoading ? "model_loading" : ("soft_" + kind.name().toLowerCase(java.util.Locale.ROOT)), kind, e, cdMs);
                AuxDownTracker.markSoft("query-transformer:runLLM", "soft-" + kind.name().toLowerCase(java.util.Locale.ROOT));
                try {
