...
public class ChatWorkflow {
    ...
    public ChatResult continueChat(ChatRequestDto req,
        ...
        try {
            ...
            // If final output is empty, fall back to evidence-only composer (if any) to ensure a visible response.
     ...
     *
     * <p>When the final answer becomes blank (e.g., LLM transient EMPTY/blank), SSE streaming
     * emits no tokens and the client may appear frozen. This guard ensures we always
     * return a non-empty response by stepping down to an evidence-only answer (when available).
     */
