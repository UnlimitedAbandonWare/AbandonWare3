...
public class HybridWebSearchEmptyFallbackAspect {
     ...
     *
     * <p>Important: we only drop low-trust entries when we still have at least one other entry
     ...
     *
     * <p>GLGO(서비스 기동 직후) 구간에서는 budget_exhausted(raw=0ms) / 429 / COOLDOWN 전환이
    ...
    private List<String> rescueEmptyHybridSearch(
        ...
        // 브레이커가 닫혀있더라도 merged=0이 반복되는 케이스(예: strictDomain starvation, coldstart timeout)
        ...
        if (coldStartBudgetExhaustedSkipLive) {
            ...
            // - Soft-skip and immediately stepdown to offline ladders (cache-only / trace-pool).
            ...
            // Canonical skip markers (so merged=0 postmortem can distinguish budget exhaustion
            ...
            try {
                ...
                if (allowDropOfficialOnly && (globalDeadlineNs - System.nanoTime()) > 0L) {
                    ...
                    try {
                        ...
                        TraceStore.put("web.failsoft.hybridEmptyFallback.demotion.nofilterSafe.skipped.reason", blockedReason);
                    } catch (Throwable ignore) {
                        // best-effort
                    }
                }
    ...
    private void recordNoRescueBreadcrumbs(
        ...
        try {
            ...
            // Final classification marker for postmortem scripts (extract true terminal merged=0).
    ...
    private FallbackAttempt attemptFallback(
        ...
        if (exec == null) {
                ...
                if (remainNs <= 0L) {
                    if (!usedMinLiveBudget && minLiveBudgetNs > 0L) {
                        ...
                        try {
                            ...
                            TraceStore.put("web.failsoft.hybridEmptyFallback.completionPoll.minLiveBudget.ms", completionPollMinLiveBudgetMs);
                        } catch (Throwable ignore) {
                            // best-effort
                        }
                    } else {
            ...
            try {
                ...
                TraceStore.put("web.failsoft.hybridEmptyFallback.completionPoll.waitedMs", TimeUnit.NANOSECONDS.toMillis(pollWaitNs));
                if (usedMinLiveBudget) {
                    TraceStore.put("web.failsoft.hybridEmptyFallback.completionPoll.minLiveBudget.waitedMs", TimeUnit.NANOSECONDS.toMillis(minLiveBudgetWaitNs));
                    TraceStore.put("web.failsoft.hybridEmptyFallback.completionPoll.minLiveBudget.yieldedCompletion", minLiveBudgetYieldedCompletion);
                }
            } catch (Throwable ignore) {
                // best-effort
            }
     ...
     *
     * <p>This is intentionally conservative and only targets common polite/request
    ...
    private List<String> maybeFilterLowTrustForNofilterSafe(List<String> merged, int topK) {
        ...
        try {
            TraceStore.put("web.failsoft.hybridEmptyFallback.demotion.nofilterSafe.lowTrustFilter.removedCount", removed);
            TraceStore.put("web.failsoft.hybridEmptyFallback.demotion.nofilterSafe.lowTrustFilter.keptCount", filtered.size());
            TraceStore.put("web.failsoft.hybridEmptyFallback.demotion.nofilterSafe.lowTrustFilter.removedHostCrc32.sample",
    ...
    private boolean isBudgetExhaustedRaw0() {
        try {
            // Prefer explicit cause marker emitted by HybridWebSearchProvider.awaitWithDeadline
            ...
            // marker exists. DO NOT infer from missing keys (TraceStore.getLong(..) defaults
